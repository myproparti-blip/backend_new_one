...
export const updateUbiShop = async (req, res) => {
    try {
        ...
        ("[updateUbiShop] Request received:", {
            id,
            username,
            userRole,
            action: "save changes",
        ...
        ("[updateUbiShop] Permission check passed. Previous status:", ubiShop.status);
...
export const managerSubmitUbiShop = async (req, res) => {
    try {
        ...
        ("[managerSubmitUbiShop] Request received:", {
            id,
            action,
            username,
            userRole,
        ...
        ("[managerSubmitUbiShop] Performing action:", {
            action,
            ubiShopId: id,
...
export const requestUbiShopRework = async (req, res) => {
    try {
        ...
        ("[requestUbiShopRework] Request:", { id, username, userRole, clientId: clientId?.substring(0, 8) });
        ...
        try {
            ...
            if (ubiShop) {
                ("[requestUbiShopRework] Found by _id");
            }
        } catch (idError) {
            ("[requestUbiShopRework] Not a valid ObjectId, trying uniqueId");
        }
        ...
        if (!ubiShop) {
            ubiShop = await UbiShopModel.findOne({ uniqueId: String(id) }).lean();
            if (ubiShop) {
                ("[requestUbiShopRework] Found by uniqueId");
            }
        }

        ("[requestUbiShopRework] Found UbiShop:", ubiShop ? ubiShop._id : "not found");
        ...
        try {
            ...
            ("[requestUbiShopRework] Not a valid ObjectId, trying uniqueId for update");
            // Fallback: try by uniqueId
        ...
        (`[deleteUbiShop] Deleted by ${username}:`, { id, clientId: clientId.substring(0, 8) + "..." });
        ...
        (`[deleteMultipleUbiShops] Deleted by ${username}:`, { count: result.deletedCount, clientId: clientId.substring(0, 8) + "..." });
