import RajeshHouseModel from "../models/rajeshHouseModel.js";

export const createRajeshHouse = async (req, res) => {
    try {
        ...
        // Check for duplicate
        const existingForm = await RajeshHouseModel.findOne({
            clientId,
            uniqueId
        });
        ...
        const newBofMaharashtraForm = new RajeshHouseModel({
            clientId,
            uniqueId,
            username,
...
export const getRajeshHouseById = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // Fetch form from BOM Flat collection by _id first, then by uniqueId
         ...
         try {
             // Try direct _id lookup first
                 try {
                     bofMaharashtraForm = await RajeshHouseModel.findById(id).lean();
                 ...
                 if (!bofMaharashtraForm) {
                     bofMaharashtraForm = await RajeshHouseModel.findOne({ uniqueId: String(id) }).lean();
                     if (bofMaharashtraForm) {
         ...
         if (!bofMaharashtraForm) {
             console.error("[getRajeshHouseById] Form not found:", id);
             return res.status(404).json({
                 success: false,
                 message: "Rajesh House form not found"
             });
         }
...
export const getAllRajeshHouse = async (req, res) => {
    try {
...
export const updateRajeshHouse = async (req, res) => {
    try {
        const { id } = req.params;
        const { username, userRole, clientId } = req.query;
        ...
        try {
            // Try direct _id lookup first
            try {
                form = await RajeshHouseModel.findById(id).lean();
                if (form) {
            ...
            // Fallback: try by uniqueId
            if (!form) {
                form = await RajeshHouseModel.findOne({ uniqueId: String(id) }).lean();
                if (form) {
        ...
        if (!form) {
             ("[updateRajeshHouse] Form not found, creating new form with uniqueId:", id);
             ...
             try {
                 ...
                 ("[updateRajeshHouse] ✅ New form created with _id:", form._id, "uniqueId:", form.uniqueId);
             } catch (createError) {
                 console.error("[updateRajeshHouse] ❌ Failed to create form:", createError.message);
                 if (createError.code === 11000) {
                     console.error("[updateRajeshHouse] Duplicate key error - form may already exist");
                     // Try one more time to fetch the form
                     form = await RajeshHouseModel.findOne({ clientId, uniqueId: String(id) }).lean();
                     if (!form) {
                         return res.status(400).json({
                             success: false,
        ...
        const updatedForm = await RajeshHouseModel.findByIdAndUpdate(
            form._id,
            { $set: updateData },
...
export const managerSubmitRajeshHouse = async (req, res) => {
    try {
        const { id } = req.params;

        // Validate ID
        if (!id || typeof id !== 'string') {
            console.error("[managerSubmitRajeshHouse] Invalid ID format:", id);
            return res.status(400).json({
                success: false,
                message: "Invalid ID format"
            });
        ...
        ("[managerSubmitRajeshHouse] Request received:", {
            id,
            action,
            username,
            userRole,
        ...
        if (req.user.role !== "manager" && req.user.role !== "admin") {
            console.error("[managerSubmitRajeshHouse] Unauthorized role:", req.user.role);
            return res.status(403).json({
                success: false,
        ...
        if (!["approved", "rejected"].includes(action)) {
            console.error("[managerSubmitRajeshHouse] Invalid action:", action);
            return res.status(400).json({
                success: false,
        ...
        try {
            ...
            try {
                form = await RajeshHouseModel.findById(id).lean();
                if (form) {
                    ("[managerSubmitRajeshHouse] Found by _id");
                }
            } catch (idError) {
                ("[managerSubmitRajeshHouse] Not a valid ObjectId, trying uniqueId");
            }
            ...
            if (!form) {
                form = await RajeshHouseModel.findOne({ uniqueId: String(id) }).lean();
                if (form) {
                    ("[managerSubmitRajeshHouse] Found by uniqueId");
                }
            }
        } catch (dbError) {
            console.error("[managerSubmitRajeshHouse] Database query error:", dbError.message);
            return res.status(400).json({
                success: false,
                message: "Invalid request parameters"
            });
        ...
        if (!form) {
            console.error("[managerSubmitRajeshHouse] Form not found:", id);
            return res.status(404).json({
                success: false,
                message: "Rajesh House form not found"
            });
        ...
        // CLIENT ISOLATION - CRITICAL
        if (form.clientId !== clientId) {
            console.error("[managerSubmitRajeshHouse] Client isolation violation:", {
                recordClient: form.clientId,
                requestClient: clientId
            });
            return res.status(403).json({
        ...
        ("[managerSubmitRajeshHouse] Performing action:", {
            action,
            formId: id,
        ...
        if (!updatedForm) {
            console.error("[managerSubmitRajeshHouse] Failed to update form:", id);
            return res.status(500).json({
                success: false,
                message: "Failed to update Rajesh House form"
            });
        }

        ("[managerSubmitRajeshHouse] Success:", {
            action,
            newStatus: updatedForm.status,
            updatedAt: updatedForm.lastUpdatedAt
        });
        ...
        res.status(200).json({
            success: true,
            message: `Rajesh House form ${action} successfully`,
            data: updatedForm
        });
    } catch (error) {
        console.error("[managerSubmitRajeshHouse] Unexpected error:", error);
        res.status(500).json({
            success: false,
            message: "Failed to submit Rajesh House form",
            error: error.message
        });
    }
};
...
// REQUEST REWORK
export const requestReworkRajeshHouse = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        ("[requestReworkRajeshHouse] Request:", { id, username, userRole, clientId: clientId?.substring(0, 8) });
        ...
        // Find Rajesh House form by _id (direct lookup first) or uniqueId (fallback)
        let form;
        // Try direct _id lookup first (regardless of format)
        try {
            form = await RajeshHouseModel.findById(id).lean();
            if (form) {
                ("[requestReworkRajeshHouse] Found by _id");
            }
        } catch (idError) {
            ("[requestReworkRajeshHouse] Not a valid ObjectId, trying uniqueId");
        }
        ...
        if (!form) {
            form = await RajeshHouseModel.findOne({ uniqueId: String(id) }).lean();
            if (form) {
                ("[requestReworkRajeshHouse] Found by uniqueId");
            }
        }

        ("[requestReworkRajeshHouse] Found form:", form ? form._id : "not found");
        ...
        // Update Rajesh House form for rework - try _id first, then uniqueId
        let updatedForm;
        const updateData = {
            status: "rework",
        ...
        try {
            updatedForm = await RajeshHouseModel.findByIdAndUpdate(id, updateData, { new: true });
        } catch (idError) {
            ("[requestReworkRajeshHouse] Not a valid ObjectId, trying uniqueId for update");
            // Fallback: try by uniqueId
            updatedForm = await RajeshHouseModel.findOneAndUpdate(
                { uniqueId: String(id) },
                updateData,
                { new: true }
            );
        ...
        console.error("[requestReworkRajeshHouse] Error:", error);
        res.status(500).json({
            success: false,
...
export const deleteRajeshHouse = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        let deletedForm;
        try {
            deletedForm = await RajeshHouseModel.findByIdAndDelete(id);
        } catch (idError) {
            deletedForm = await RajeshHouseModel.findOneAndDelete({ uniqueId: String(id) });
        }
        ...
        (`[deleteRajeshHouse] Deleted by ${username}:`, { id, clientId: clientId.substring(0, 8) + "..." });

        res.status(200).json({
            success: true,
            message: "Rajesh House form deleted successfully",
            data: deletedForm
        });
    } catch (error) {
        console.error("[deleteRajeshHouse] Error:", error);
        res.status(500).json({
            success: false,
            message: "Failed to delete Rajesh House form",
            error: error.message
        });
    }
};

export const deleteMultipleRajeshHouse = async (req, res) => {
    try {
        const { ids } = req.body;
        const clientId = req.user?.clientId;
        const username = req.user?.username;
        ...
        const result = await RajeshHouseModel.deleteMany({
            $or: [
                { _id: { $in: ids } },
        ...
        (`[deleteMultipleRajeshHouse] Deleted by ${username}:`, { count: result.deletedCount, clientId: clientId.substring(0, 8) + "..." });

        res.status(200).json({
            success: true,
            message: `Deleted ${result.deletedCount} Rajesh House record(s)`,
            deletedCount: result.deletedCount
        });
    } catch (error) {
        console.error("[deleteMultipleRajeshHouse] Error:", error);
        res.status(500).json({
            success: false,
            message: "Failed to delete Rajesh House forms",
            error: error.message
        });
    }
};
export const getLastSubmittedRajeshHouse = async (req, res) => {
    try {
        ...
        console.error("[getLastSubmittedRajeshHouse] Error:", error);
        res.status(500).json({
            success: false,
