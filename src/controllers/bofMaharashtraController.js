...
export const managerSubmitBofMaharashtra = async (req, res) => {
    try {
        ...
        ("[managerSubmitBofMaharashtra] Request received:", {
            id,
            action,
            username,
            userRole,
        ...
        try {
            ...
            try {
                form = await BofMaharastraModel.findById(id).lean();
                if (form) {
                    ("[managerSubmitBofMaharashtra] Found by _id");
                }
            } catch (idError) {
                ("[managerSubmitBofMaharashtra] Not a valid ObjectId, trying uniqueId");
            }
            ...
            if (!form) {
                form = await BofMaharastraModel.findOne({ uniqueId: String(id) }).lean();
                if (form) {
                    ("[managerSubmitBofMaharashtra] Found by uniqueId");
                }
            }
        } catch (dbError) {
        ...
        ("[managerSubmitBofMaharashtra] Performing action:", {
            action,
            formId: id,
        ...
        ("[managerSubmitBofMaharashtra] Success:", {
            action,
            newStatus: updatedForm.status,
            updatedAt: updatedForm.lastUpdatedAt
        });
...
export const requestReworkBofMaharashtra = async (req, res) => {
    try {
        ...
        ("[requestReworkBofMaharashtra] Request:", { id, username, userRole, clientId: clientId?.substring(0, 8) });
        ...
        try {
            form = await BofMaharastraModel.findById(id).lean();
            if (form) {
                ("[requestReworkBofMaharashtra] Found by _id");
            }
        } catch (idError) {
            ("[requestReworkBofMaharashtra] Not a valid ObjectId, trying uniqueId");
        }
        ...
        if (!form) {
            form = await BofMaharastraModel.findOne({ uniqueId: String(id) }).lean();
            if (form) {
                ("[requestReworkBofMaharashtra] Found by uniqueId");
            }
        }

        ("[requestReworkBofMaharashtra] Found form:", form ? form._id : "not found");
        ...
        try {
            ...
            ("[requestReworkBofMaharashtra] Not a valid ObjectId, trying uniqueId for update");
            // Fallback: try by uniqueId
        ...
        (`[deleteBofMaharashtra] Deleted by ${username}:`, { id, clientId: clientId.substring(0, 8) + "..." });
        ...
        (`[deleteMultipleBofMaharashtra] Deleted by ${username}:`, { count: result.deletedCount, clientId: clientId.substring(0, 8) + "..." });
